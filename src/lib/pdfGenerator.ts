...
const generateDoc = (doc: jsPDF, quotation: Quotation) => {
  ...
  addTotalRow('Subtotal:', formatCurrency(quotation.subtotal));
  addTotalRow(`GST (${quotation.gstRate}%):`, formatCurrency(quotation.gst));
  ...
  // Grand Total
  doc.setFillColor(...COLORS.accent);
  doc.roundedRect(totalsX - 5, yPos - 4, totalsWidth + 5, 12, 1, 1, 'F');
  addText('Total Payable:', totalsX, yPos + 4, 10, 'bold', COLORS.white);
  addText(formatCurrency(quotation.totalPayable), pageWidth - margin, yPos + 4, 11, 'bold', COLORS.white, 'right');
